"""Team-wide Azure DevOps / import sync logic, shared by the Analysis views
and the sync_azure_devops management command."""
...
def run_team_ado_sync(settings_obj):
    """Run the team-wide Azure DevOps sync and persist the outcome on settings_obj.
    ...
    result = import_team_work_items(fetched, source=WorkItem.Source.AZURE_DEVOPS, upsert=True)
    settings_obj.last_synced_at = timezone.now()
